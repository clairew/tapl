...
data Context = Context
    ...
    , typeVars :: Map.Map String (Kind, Type)
    } deriving (Show, Eq)
...
extendTypeVar var k (Context terms types) = Context terms (Map.insert var (k, bound) types)
...
lookupTypeVarKind x (Context _ types) = snd <$> Map.lookup x types
...
kindOf ctx (TAbs v k1 typ) = do
    let extended = extendTypeVar v k1 TTop ctx
